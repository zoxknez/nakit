    ...
    <div className="min-h-screen bg-gradient-to-b from-brand-dark via-brand-primary/20 to-brand-dark relative overflow-hidden">
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-2 gap-12 lg:gap-16">
          {/* Details Section (First on mobile, second on desktop) */}
          <div className="order-1 lg:order-2 lg:sticky lg:top-24 lg:h-fit">
            <div className="relative bg-brand-dark/40 backdrop-blur-xl rounded-3xl p-6 md:p-10 shadow-3xl shadow-black/80 border border-brand-secondary/20 leather-texture overflow-hidden">
              ...
              <div className="relative z-10 space-y-6 md:space-y-8">
                ...
                <div className="space-y-4">
                  <div className="flex flex-col gap-4">
                    ...
                    {piece.price && (
                      <div className="flex flex-col items-start border-l-4 border-brand-secondary/40 pl-6 py-2 bg-brand-secondary/5 rounded-r-2xl shadow-inner max-w-fit">
                ...
                <div className="relative bg-brand-dark/30 backdrop-blur-sm rounded-2xl p-6 md:p-8 border border-brand-secondary/30 shadow-inner">
                  <div className="absolute -top-3 left-8 px-4 py-1 bg-brand-secondary rounded-full shadow-lg shadow-brand-secondary/20">
                    <span className="text-brand-dark text-xs font-bold uppercase tracking-wider">
                      {locale === 'sr' ? 'Ručna Izrada' : locale === 'ru' ? 'Ручная Работа' : 'Handcrafted'}
                    </span>
                  </div>
                  <ul className="grid sm:grid-cols-2 lg:grid-cols-1 gap-x-6 gap-y-4 mt-4">
                    <li className="flex items-start gap-4 group">
                      <div className="mt-1 w-6 h-6 rounded-full bg-brand-secondary/10 flex items-center justify-center flex-shrink-0 border border-brand-secondary/20 shadow-md">
                        <svg className="w-3 h-3 text-brand-secondary" fill="currentColor" viewBox="0 0 20 20">
                          <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                        </svg>
                      </div>
                      <span className="text-brand-accent/90 font-serif leading-relaxed text-sm md:text-base">
                        {locale === 'sr' ? 'Proces izrade: ~3 dana' : locale === 'ru' ? 'Процесс изготовления: ~3 дня' : 'Creation process: ~3 days'}
                      </span>
                    </li>
                    <li className="flex items-start gap-4 group">
                      <div className="mt-1 w-6 h-6 rounded-full bg-brand-secondary/10 flex items-center justify-center flex-shrink-0 border border-brand-secondary/20 shadow-md">
                      ...
                      <span className="text-brand-accent/90 font-serif leading-relaxed text-sm md:text-base">
                        {locale === 'sr' ? 'Potpuno ručno izrađeno' : locale === 'ru' ? 'Полностью ручная работа' : 'Completely handmade'}
                      </span>
                    </li>
                  </ul>
                </div>

                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-4 border-t border-brand-secondary/10">
                  <div className="flex items-center gap-2 text-sm text-brand-accent/60">
                    ...
                    <span className="font-serif tracking-wide truncate">
                      {locale === 'sr' ? 'Kreirano' : locale === 'ru' ? 'Создано' : 'Created'}{' '}
                      <span className="text-brand-secondary font-bold">
                        {new Date(piece.createdAt).toLocaleDateString(locale, { year: 'numeric', month: 'long', day: 'numeric' })}
                      </span>
                    </span>
                  </div>
                </div>
          ...
          {/* Gallery Column (Second on mobile, first on desktop) */}
